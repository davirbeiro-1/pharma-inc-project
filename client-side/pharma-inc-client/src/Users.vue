    ...
    <b-card class="login-card shadow bg-white rounded">
      ...
      <b-card-body>
        <PrimaryCardInfo></PrimaryCardInfo>
          ...
          <FilterData @onFilterTextChanged="filterTextChanged($event)"></FilterData>
...
import PrimaryCardInfo from "@/components/PrimaryCardInfo.vue";
  ...
  methods: {
    ...
    async handleSubmit() {
      ...
      if (this.$v.$invalid) {
        ...
        this.submitted = true;
        this.isLoading = true;
        await userService.updateUserById(this.user);
